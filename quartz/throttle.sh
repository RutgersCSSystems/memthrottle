...
RUNSTREAM() {
  #Compile stream
  cd $BASE/bench/stream && make clean && make
  numactl --membind=0 $BASE/bench/stream/stream_c.exe &> $BASE/throttle.out
  numactl --membind=1 $BASE/bench/stream/stream_c.exe &>> $BASE/throttle.out
  cd $BASE
}
...
PERFORM_THROTTLE_QUARTZ() {
     export QUARTZSCRIPTS=$BASE/scripts
     ...
     if [ -f $BASE/bandwidth_model ]; then
         cp $BASE/bandwidth_model /tmp/
     else
         cp /tmp/bandwidth_model $BASE/	
     fi

     if [ -f $BASE/mc_pci_bus ]; then
         cp $BASE/mc_pci_bus /tmp/
     else
         cp /tmp/mc_pci_bus $BASE/
     fi
}
...
grep -r "Copy:" $BASE/throttle.out | awk '{print $2}'
echo "-----------------"
echo " "
